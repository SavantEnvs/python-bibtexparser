...
#      then install atheris + pytest + pyparsing OFFLINE from that wheelhouse into a fixed
...
#      but needs pyparsing (its one runtime dep at this commit's setup.py) from the wheelhouse.
...
#    manylinux wheel for this CPython. pytest runs bibtexparser's own suite; pyparsing is
#    bibtexparser's sole runtime dependency at this commit (setup.py: pyparsing>=2.0.3).
# Pinned below 3.1: pyparsing 3.1 (mid-2023, after this commit) added a parseString ->
# parse_string deprecation warning on every call, which breaks the suite's own warning-count
# assertions (test_bparser.py) — an artifact of a newer dependency, not this commit's code.
PKGS=(atheris pytest "pyparsing>=2.0.3,<3.1")
need_download=0
...
#    site dir holds atheris+pytest+pyparsing we SKIP the reinstall. bibtexparser itself stays the
#    editable source tree (repo root on PYTHONPATH) so a PATCH agent's edits under bibtexparser/
#    take effect with no reinstall.
if "$PY" -c "import os,glob,sys; sys.exit(0 if (glob.glob(os.path.join('$SITE','atheris*')) and glob.glob(os.path.join('$SITE','pytest*')) and glob.glob(os.path.join('$SITE','pyparsing*'))) else 1)"; then
